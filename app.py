...
- 소형기 청소 약 30분 / 팀 단위 (소형기 10~13명) / 초보자 가능 (교육 제공) / 남자 여자 무관 / 연령무관 / 학별 무관
...
- 단시간의 경우 셔틀버스는 지원되지 않습니다. """,

    "단시간": """⏰ **단시간조 안내**

현재 모집 중인 단시간조 정보는 왼쪽 패널에서 확인해 주세요.

더 자세한 문의는 💬 **카카오 오픈채팅**으로 연락해 주세요!""",
}

KEYWORD_MAP_FULL = [
    (["급여", "월급", "돈", "페이", "연봉", "수당", "얼마", "임금", "월수입", "연봉"], "급여"),
    (["근무시간", "몇시", "출근", "퇴근", "스케줄", "시간대", "휴무", "쉬는날", "근무형태"], "근무시간"),
    (["셔틀", "버스", "차량", "픽업", "발산", "부천", "원종", "고강", "김포"], "셔틀"),
    (["복리", "복지", "혜택", "항공권", "유니폼", "식비", "식대", "베네핏"], "복리후생"),
    (["업무", "하는일", "청소", "정비", "기내", "직무", "일이", "어떤일", "무슨일"], "업무"),
    (["근무지", "위치", "어디", "공항", "인천"], "근무지"),
    (["지원", "신청", "어떻게", "방법", "접수", "지원서"], "지원"),
    (["면접", "합격", "서류", "연락", "전형"], "면접"),
    (["주차", "주차장", "차"], "주차"),
    (["교통비", "교통", "지원비", "교통지원"], "교통비"),
    (["초보", "경력", "처음", "무경력", "배우", "신입"], "초보"),
    (["단시간", "파트", "시간제", "알바", "단기"], "단시간"),
]

def get_rule_reply(user_input: str) -> str:
    text = user_input.replace(" ", "")
    active = get_active_shifts()

    for keywords, key in KEYWORD_MAP_FULL:
        if any(kw in text for kw in keywords):
            # 단시간조 활성 여부에 따라 급여/복리후생 답변 분기
            if key == "급여" and active:
                return DIRECT_ANSWERS["급여_단시간"]
            if key == "복리후생" and active:
                return DIRECT_ANSWERS["복리후생_단시간"]
            if key in DIRECT_ANSWERS:
                return DIRECT_ANSWERS[key]

    return """죄송합니다, 해당 내용은 담당자에게 직접 문의해 주세요 😊

💬 **카카오 오픈채팅**으로 문의하시면 빠르게 안내해드릴 수 있어요!

아래 버튼을 눌러 자주 묻는 질문을 확인하실 수도 있어요.
- 💰 급여 안내 / ⏰ 근무시간 / 🚌 셔틀 시간 / 🎁 복리후생"""
...
with col_info:
    ...
    with st.expander("🚌 셔틀버스"):
        st.markdown("""
        <div class="info-card"><strong>🚌 셔틀버스</strong><br>발산역 / 김포공항 04:30<br>부천 원종사거리 04:30<br>고강동 지원<br><br>※ 셔틀 이용 시 교통비에서 셔틀비용 차감 후 지급 
        ※단시간의 경우 셔틀버스 지원되지 않습니다. </div>
        """, unsafe_allow_html=True)
...
with col_chat:
    st.markdown("#### 💬 채용 상담")
    st.caption("버튼을 누르거나 궁금한 점을 직접 입력해 주세요 😊")
...
오전 05:45 근무 기준 **04:30 이전 출발**을 권장드립니다 😊"""
    ...
    # 직접 입력 → 규칙 기반 답변
    if user_input := st.chat_input("궁금한 점을 입력하세요. 예) 초보자도 지원 가능한가요?"):
        matched_topic = "기타"
        for keywords, key in KEYWORD_MAP:
            if any(kw in user_input.replace(" ", "") for kw in keywords):
                matched_topic = key
                break
        reply = get_rule_reply(user_input)
