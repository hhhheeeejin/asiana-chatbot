...
# Supabase 함수
# ─────────────────────────────────────────
def save_question_log(question_text: str, question_type: str, topic: str, reply_text: str = ""):
    try:
        requests.post(
            f"{SUPABASE_URL}/rest/v1/question_logs",
            headers=HEADERS_WRITE,
            json={"유형": question_type, "주제": topic, "질문내용": question_text, "답변내용": reply_text},
            timeout=5
        )
    except Exception:
        pass
...
오전 05:45 근무 기준 **04:30 이전 출발**을 권장드립니다 😊"""
            ...
            save_question_log(user_msg, "버튼", "출퇴근소요시간", reply)
        else:
