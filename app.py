...
# Supabase 함수
# ─────────────────────────────────────────
def save_question_log(question_text: str, question_type: str, topic: str, reply_text: str = "", region: str = ""):
    try:
        requests.post(
            f"{SUPABASE_URL}/rest/v1/question_logs",
            headers=HEADERS_WRITE,
            json={"유형": question_type, "주제": topic, "질문내용": question_text, "답변내용": reply_text, "출발지역": region},
            timeout=5
        )
    except Exception:
        pass
...
def get_job_info() -> str:
    ...
    try:
        res = requests.get(
            f"{SUPABASE_URL}/rest/v1/job_info?id=eq.regular&select=content",
            headers=HEADERS, timeout=5
        )
        if res.status_code != 200:
            return ""
        data = res.json()
        return data[0]["content"] if data and data[0]["content"] else ""
    except Exception:
        return ""

def save_job_info(content: str):
    """정규조 추가 채용 정보 저장"""
    try:
        requests.patch(
            f"{SUPABASE_URL}/rest/v1/job_info?id=eq.regular",
            headers={**HEADERS_WRITE, "Prefer": "resolution=merge-duplicates,return=minimal"},
            json={"content": content, "updated_at": datetime.now().isoformat()},
            timeout=5
        )
    except Exception:
        pass
...
- 셔틀버스: 단시간조는 셔틀버스 지원 불가
- 모집 시간대:
{shift_list}
"""

    # 관리자가 추가한 정규조 정보
    extra_info = st.session_state.get("job_info_content", "")
...
- 지원자격: 성별 무관 / 연령 무관 / 학력 무관 / 누구나 지원 가능
...
- 고용형태: 아시아나 에어포트 직접 고용 정규직
- 지원: 간편지원 바로가기 버튼 클릭 / 면접: 서류 합격자 개별 연락{extra_section}{part_time_section}
...
def get_ai_reply(messages: list) -> str:
    try:
        ...
        return "일시적인 오류가 발생했습니다. 잠시 후 다시 시도하거나 💬 카카오 오픈채팅으로 문의해 주세요 😊"
...
FAQ_ANSWERS = {
    "급여": DIRECT_ANSWERS["급여"],
    "급여_단시간": DIRECT_ANSWERS["급여_단시간"],
    "셔틀": DIRECT_ANSWERS["셔틀"],
    "복리후생": DIRECT_ANSWERS["복리후생"],
    "복리후생_단시간": DIRECT_ANSWERS["복리후생_단시간"],
}
...
if "job_info_content" not in st.session_state:
    st.session_state.job_info_content = get_job_info()
    ...
    if active_shifts:
        ...
        with st.expander("⏰ 모집 시간대 · 급여 · 고용형태"):
            ...
            <div class="part-card">
                ...
                <strong>🏢 고용형태</strong><br>윌앤비전 정규직<br>(4대보험, 기본 복리후생 적용)<br><br>
                <strong>🚌 셔틀버스</strong><br>🚫 지원 불가 (개인 이동 수단 이용)
            </div>
            """, unsafe_allow_html=True)
...
with col_chat:
    ...
    with c1:
        if st.button("💰 급여 안내", use_container_width=True):
            ...
            save_question_log("급여가 어떻게 되나요?", "버튼", "급여",
                              FAQ_ANSWERS["급여_단시간"] if get_active_shifts() else FAQ_ANSWERS["급여"])
    with c2:
        if st.button("🚌 셔틀 시간", use_container_width=True):
            st.session_state.faq_trigger = "셔틀"
            st.session_state.show_commute = False
            save_question_log("셔틀버스 시간이 어떻게 되나요?", "버튼", "셔틀", FAQ_ANSWERS["셔틀"])
    with c3:
        if st.button("🎁 복리후생", use_container_width=True):
            ...
            save_question_log("복리후생 혜택을 알려주세요.", "버튼", "복리후생",
                              FAQ_ANSWERS["복리후생_단시간"] if get_active_shifts() else FAQ_ANSWERS["복리후생"])
    with c4:
    ...
    if st.session_state.show_commute:
        with st.container(border=True):
            ...
            st.caption("출발지를 입력하시면 자차/대중교통/셔틀 예상 소요시간을 안내해드립니다.")
            addr_col, btn_col = st.columns([3, 1])
            with addr_col:
                home_addr = st.text_input("출발지", placeholder="예) 부천 원종동, 강남구, 수원시, 인천 부평 등",
    ...
    if st.session_state.faq_trigger:
        ...
        if trigger.startswith("__commute__"):
            ...
            if any(k in region for k in ["발산", "김포"]):
                shuttle_info = "발산역 / 김포공항에서 **04:30** 출발 셔틀 이용 가능 ✅"
            elif any(k in region for k in ["부천", "원종", "고강"]):
                shuttle_info = "부천 원종사거리에서 **04:30** 출발 셔틀 이용 가능 ✅"
            else:
                shuttle_info = "해당 지역 셔틀버스 없음 (자차 또는 대중교통 이용)"

            # AI로 소요시간 계산
            commute_prompt = f"""
사용자가 "{region}" 에서 인천국제공항(제1터미널 기준)까지 출퇴근하려고 합니다.

아래 형식으로 친절하게 답변해주세요:

🗺️ **{region} → 인천국제공항 출퇴근 안내**

🚗 **자차**
- 예상 소요시간: (대략적인 시간, 교통상황에 따라 다를 수 있음 명시)
- 고속도로/주요 경로 간단히 안내
- 주차: 야외 주차장 이용 가능 (월 3만원)

🚇 **대중교통**
- 예상 소요시간: (대략적인 시간)
- 주요 경로: (지하철/버스/공항철도 등 핵심 경로만 간단히)

🚌 **셔틀버스**
- {shuttle_info}

⏰ **출근 권장 출발 시간**
- 오전 05:45 근무 기준 몇 시에 출발해야 하는지 안내

마지막에 "정확한 시간은 실시간 교통상황에 따라 다를 수 있습니다 😊" 추가
"""
            try:
                from openai import OpenAI
                client = OpenAI(api_key=st.secrets["OPENAI_API_KEY"])
                response = client.chat.completions.create(
                    model="gpt-4o-mini",
                    messages=[{"role": "user", "content": commute_prompt}],
                    max_tokens=800,
                )
...
오전 05:45 근무 기준 **04:30 이전 출발**을 권장드립니다 😊"""
            ...
            save_question_log(user_msg, "버튼", "출퇴근소요시간", reply, region=region)
        else:
        ...
        with chat_container:
            ...
            with st.chat_message("assistant"):
                with st.spinner("답변 생성 중..."):
                    ...
                    st.markdown(reply)
                    st.session_state.messages.append({"role": "assistant", "content": reply})
                    save_question_log(user_input, "직접입력", matched_topic, reply)
...
st.markdown("---")
st.markdown("""
<div style="background:#f8f9fa; border-radius:10px; padding:20px 24px; font-size:0.78rem; color:#555; line-height:2;">
<strong style="color:#333;">📋 채용 관련 안내사항</strong><br><br>
※ 제출 된 채용 서류는 반환청구에 대비하여 최대 90일간 보관합니다.<br>
※ 최종 합격자 발표 후 90일 이내 채용서류 반환 요청 시 본인 확인 후 제11조 제1항 절차에 따라 반환이 가능합니다.<br>
※ 반환 청구기간 이후에는 개인정보보호법에 의거하여 자동 파기처리 합니다.<br>
※ 보관기간이 초과 된 채용서류는 자동 파기되며, 회사에 책임 없는 사유로 채용 서류 멸실 된 경우 반환의무가 이행 된 것으로 봅니다.<br>
※ 채용서류 반환 비용은 구인자 부담이 원칙이나 예외적인 경우 그 비용을 구직자가 부담 할 수 있습니다.<br>
※ 홈페이지 또는 전자우편으로 제출 된 경우나 자발적으로 제출 된 서류는 상기 내용에도 불구하고 채용절차법 제11조 제1항 단서에 따라 반환 의무가 없습니다.<br>
※ 제출 서류에 허위 사실이 있는 경우 채용이 어려울 수 있습니다.<br>
※ 본 공고는 사전 통지 없이 변경/마감될 수 있습니다.<br>
※ 본 채용 과정에서는 효율적인 서류 검토를 위해 인공지능(AI) 기술이 일부 활용될 수 있으며, 입력된 데이터는 채용 목적 이외에 절대 사용되지 않습니다.
</div>
""", unsafe_allow_html=True)
...
if not st.session_state.admin_auth:
    ...
    with st.expander("🔓 관리자 메뉴 (로그인됨)", expanded=True):
        ...
        if not df_questions.empty:
            ...
            today_count = len(df_questions[df_questions["created_at"].astype(str).str.startswith(today)]) if "created_at" in df_questions.columns else 0
            button_count = len(df_questions[df_questions["유형"] == "버튼"])
            direct_count = len(df_questions[df_questions["유형"] == "직접입력"])

            m1, m2, m3, m4 = st.columns(4)
            m1.metric("📩 총 질문 수", f"{total}건")
            m2.metric("📅 오늘 질문", f"{today_count}건")
            m3.metric("🖱️ 버튼 클릭", f"{button_count}건")
            m4.metric("⌨️ 직접 입력", f"{direct_count}건")
            ...
            with col_chart1:
                st.markdown("**주제별 질문 분포**")
                topic_counts = df_questions["주제"].value_counts().reset_index()
                topic_counts.columns = ["주제", "질문수"]
                st.bar_chart(topic_counts.set_index("주제"))

            with col_chart2:
                st.markdown("**유형별 질문 분포**")
                type_counts = df_questions["유형"].value_counts().reset_index()
                type_counts.columns = ["유형", "건수"]
                st.bar_chart(type_counts.set_index("유형"))

            # 출발지역 통계
            df_commute = df_questions[
                (df_questions["주제"] == "출퇴근소요시간") &
                (df_questions["출발지역"].notna()) &
                (df_questions["출발지역"] != "")
            ] if "출발지역" in df_questions.columns else pd.DataFrame()

            if not df_commute.empty:
                st.markdown("**🗺️ 출퇴근 문의 지역 TOP 10**")
                region_counts = df_commute["출발지역"].value_counts().reset_index().head(10)
                region_counts.columns = ["지역", "문의수"]
                col_region1, col_region2 = st.columns(2)
                with col_region1:
                    st.dataframe(region_counts, use_container_width=True, hide_index=True)
                with col_region2:
                    st.bar_chart(region_counts.set_index("지역"))

            # 최근 질문 10건
            st.markdown("**🕐 최근 질문 10건**")
            recent_cols = ["created_at", "유형", "주제", "질문내용"]
            recent_cols = [c for c in recent_cols if c in df_questions.columns]
            st.dataframe(df_questions[recent_cols].head(10), use_container_width=True)

            # 전체 로그 + 다운로드
            with st.expander("📋 전체 질문 내역 보기"):
        ...
        # ── 정규조 추가 채용 정보 ─────────────
        st.markdown("---")
        st.markdown("##### 📝 정규조 추가 채용 정보")
        st.caption("여기에 입력한 내용이 AI 답변에 자동으로 반영됩니다.")

        job_info_input = st.text_area(
            "추가 정보 입력",
            value=st.session_state.job_info_content,
            height=200,
            placeholder="예)\n- 항공기 기내 쓰레기 수거 및 분리수거\n- 승객 좌석 시트커버 교체\n- 화장실 청소 및 소독\n- 수습기간: 3개월\n- 계약기간: 1년 후 정규직 전환",
            label_visibility="collapsed"
        )
        if st.button("💾 저장하기", use_container_width=True):
            save_job_info(job_info_input)
            st.session_state.job_info_content = job_info_input
            st.success("✅ 저장 완료! AI 답변에 즉시 반영됩니다.")
